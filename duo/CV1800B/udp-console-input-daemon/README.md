...
每个 UDP 包一行，支持以下三种格式。如果一行不以 `echo ` / `cmd ` / `combo `
开头，则整行按 `echo ` 处理。

### `echo <text>`

把后面的文字逐字符“键入”到 Duo 的命令行（就像 shell 里的 `echo`）。

```bash
echo "echo hello world" | nc -u -w 0 192.168.42.1 25251
```
...
```bash
# 推荐：加 -w 0
echo "echo hello" | nc -u -w 0 192.168.42.1 25251

# 或者指定 1 秒超时
echo "echo hello" | nc -u -w 1 192.168.42.1 25251
```
...
镜像启动参数里加了 `fbcon=font:6x8`，使用内核内置的 6×8 字体，
在 240×320 的 ST7789V 屏幕上能显示更多内容。
