...
#define DEFAULT_SOCKET "/tmp/gc9a01-face-daemon.sock"
...
static inline void put_pixel(uint8_t *buf, int x, int y,
                             uint8_t r, uint8_t g, uint8_t b)
{
    if (x < 0 || y < 0 || x >= GC9A01_WIDTH || y >= GC9A01_HEIGHT)
        return;
    size_t off = ((size_t)y * GC9A01_WIDTH + (size_t)x) * 2;
    uint16_t pixel = (uint16_t)(((r & 0xf8) << 8) |
                                ((g & 0xfc) << 3) |
                                (b >> 3));
    buf[off] = (uint8_t)(pixel >> 8);
    buf[off + 1] = (uint8_t)(pixel & 0xff);
}

static void clear_buf(uint8_t *buf)
{
    memset(buf, 0, GC9A01_BUF_BYTES);
}

static void draw_bitmap_frame(uint8_t *buf, const struct render_cfg *cfg,
                              const struct frame_step *step)
{
    ...
    if (face_w > (uint32_t)GC9A01_WIDTH)
        face_w = GC9A01_WIDTH;
    if (face_h > (uint32_t)GC9A01_HEIGHT)
        face_h = GC9A01_HEIGHT;

    clear_buf(buf);
    x0 = (GC9A01_WIDTH - (int)face_w) / 2 + step->dx;
    y0 = (GC9A01_HEIGHT - (int)face_h) / 2 + step->dy;
    ...
    for (y = 0; y < face_h; y++) {
        ...
        for (x = 0; x < face_w; x++) {
            ...
            if (step->invert)
                on = !on;
            if (on)
                put_pixel(buf, x0 + (int)x, y0 + (int)y, r, g, b);
        }
    }
}
...
static int build_public_message(const char *input, char *out, size_t out_size)
{
    ...
    if (!cmd)
        return -1;
    if (strcasecmp(cmd, "gc9a01-face-daemon") == 0) {
        cmd = strtok(NULL, " \t\r\n");
        if (!cmd)
            return -1;
    }
...
static int run_daemon(struct render_cfg *cfg)
{
    ...
    if (gc9a01_hw_init(&hw, cfg->spi_path, cfg->gpio_dc, cfg->gpio_rst,
                       cfg->spi_speed_hz) < 0)
        return 1;

    framebuf = (uint8_t *)malloc(GC9A01_BUF_BYTES);
    if (!framebuf) {
        fprintf(stderr, "malloc frame buffer failed\n");
        gc9a01_hw_deinit(&hw);
        return 1;
    }
    memset(framebuf, 0, GC9A01_BUF_BYTES);
    ...
    printf("GC9A01 face daemon: spi=%s, socket=%s, udp=%u, fps=%u, face=%ux%u, default=%s\n",
           cfg->spi_path, cfg->socket_path, cfg->udp_port, cfg->fps,
           cfg->face_w, cfg->face_h, pb.default_name);

    /* Draw the first frame before entering the loop. */
    {
        const struct frame_step *step = playback_tick(&pb);
        draw_bitmap_frame(framebuf, cfg, step);
        gc9a01_hw_flush(&hw, framebuf);
        next_frame = now_ms();
    }
...
{
    printf("Usage:\n");
    printf("  %s daemon [--spi /dev/spidev0.1] [--gpio-dc N] [--gpio-rst N] [--socket PATH] [--udp-port N] [--fps N] [--width N] [--height N] [--color RRGGBB] [--default NAME] [--spi-speed HZ]\n", argv0);
...
int main(int argc, char **argv)
{
    ...
    if (strcmp(argv[1], "daemon") == 0) {
        for (i = 2; i < argc; i++) {
            if (strcmp(argv[i], "--spi") == 0 && i + 1 < argc) {
                cfg.spi_path = argv[++i];
            } else if (strcmp(argv[i], "--gpio-dc") == 0 && i + 1 < argc) {
                if (parse_int_arg(argv[++i], &cfg.gpio_dc) < 0)
                    return 1;
            } else if (strcmp(argv[i], "--gpio-rst") == 0 && i + 1 < argc) {
                if (parse_int_arg(argv[++i], &cfg.gpio_rst) < 0)
                    return 1;
                ...
                if (parse_u32_arg(argv[++i], &cfg.spi_speed_hz) < 0)
                    return 1;
            } else {
                usage(argv[0]);
                return 1;
